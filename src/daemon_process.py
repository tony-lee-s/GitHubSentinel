...
def github_job(subscription_manager, github_client, report_generator, notifier, days):
    ...
    for repo in subscriptions:
        ...
        notifier.notify(f"[GitHubSentinel]{repo} 进展简报", report)
    LOG.info(f"[定时任务执行完毕]")
...
def main():
    ...
    hacker_news_job(hacker_news_client, llm, notifier)
    ...
    schedule.every(2).hours.do(hacker_news_job, hacker_news_client, llm, notifier)
